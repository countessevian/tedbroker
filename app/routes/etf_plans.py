...
from app.database import get_collection, ETF_PLANS_COLLECTION, USER_INVESTMENTS_COLLECTION, USERS_COLLECTION, TRANSACTIONS_COLLECTION
from app.schemas import ETFPlanResponse, InvestInPlanRequest
...
async def activate_etf_plan(
    ...
    try:
        ...
        if not plan.get("is_active", True):
            raise HTTPException(status_code=400, detail="This ETF plan is not currently active")
        ...
        transaction_doc = {
            ...
            "description": f"Investment in ETF Plan: {plan['name']}",
        ...
        return {
            "message": "ETF plan activated successfully",
            "investment_id": str(result.inserted_id),
            "amount_invested": request.amount,
            "plan_name": plan["name"],
